...
export function parse_npcs() {
  const NPCCardList = csv.parse(fs.readFileSync(game_dir('rawexd', 'TripleTriad.csv')).toString(), {
    from_line: 4,
    on_record: (record, options) => {
      ...
      const guaranteed = [record.fixed_0, record.fixed_1, record.fixed_2, record.fixed_3, record.fixed_4].filter(
        is_not_zero
      );
  ...
  const BaseNpcList = csv.parse(fs.readFileSync(game_dir('rawexd', 'ENpcBase.csv')).toString(), {
    from_line: 5,
    on_record: (record, options) => {
  ...
  const BaseNPCNameList = csv.parse(fs.readFileSync(game_dir('rawexd', 'ENpcResident.csv')).toString(), {
    from_line: 5,
    on_record: (record, options) => {
      if (!record.name) {
        return null;
  ...
  const NpcNameMap = list.to_map((npc) => npc.TripleTriadNpcId, BaseNPCNameList);
