...
export function parse_npcs() {
  const NPCCardList = csv.parse(
    fs.readFileSync(game_dir("rawexd", "TripleTriad.csv")).toString(),
    {
      from_line: 4,
  ...
  const BaseNpcList = csv.parse(
    fs.readFileSync(game_dir("rawexd", "ENpcBase.csv")).toString(),
    {
      from_line: 5,
      on_record: (record, options) => {
  ...
  const BaseNPCNameList = csv.parse(
    fs.readFileSync(game_dir("rawexd", "ENpcResident.csv")).toString(),
    {
      from_line: 5,
      on_record: (record, options) => {
        if (!record.name) {
