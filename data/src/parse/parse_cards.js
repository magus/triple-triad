...
export function parse_cards() {
  const CardNameList = csv.parse(fs.readFileSync(game_dir('rawexd', 'TripleTriadCard.csv')).toString(), {
    from_line: 5,
    ...
    columns: ['id', 'name', '', '', '', 'StartsWithVowel', '', '', '', 'Description'],
  });
  ...
  const CardList = csv.parse(fs.readFileSync(game_dir('rawexd', 'TripleTriadCardResident.csv')).toString(), {
    from_line: 5,
    on_record: (record, options) => {
